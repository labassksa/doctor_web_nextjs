...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setToken(localStorage.getItem("labass_doctor_token"));
    setUserId(localStorage.getItem("labass_doctor_userId"));
  ...
  useEffect(() => {
    // Only check authentication after we've attempted to load from localStorage
    if (isAuthLoaded && (!token || !userId || !consultationId)) {
      router.push("/login");
    }
  }, [isAuthLoaded, token, userId, consultationId, router]);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100">
            ...
            {status === ConsultationStatus.Paid && (
              <div className="flex-1 sm:flex-none p-2">
                <button
                  ...
                  className="w-full sm:w-auto bg-green-600 text-white text-xs py-2 sm:py-3 px-4 sm:px-6 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                >
              ...
              <>
                <div className="flex-1 sm:flex-none p-2">
                  <button
                    ...
                    className="w-full sm:w-auto bg-red-600 text-white text-xs py-2 sm:py-3 px-4 sm:px-6 rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
                  >
                    End Consultation
                  </button>
                </div>
          ...
          {consultationInfo && (
            <div className="flex flex-col space-y-2">
              ...
              <div className="px-2 pb-2 flex gap-2">
                ...
                {consultationInfo.obesitySurvey && consultationInfo.obesitySurvey.survey && consultationInfo.obesitySurvey.survey.length > 0 && (
                  <button
                    onClick={() => setShowObesitySurvey(true)}
                    className="bg-purple-500 hover:bg-purple-600 text-white text-xs py-2 px-4 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 relative z-10"
                  >
      ...
      {/* Obesity Survey Modal */}
      {showObesitySurvey && consultationInfo?.obesitySurvey && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-3xl max-h-[90vh] overflow-hidden">
            {/* Modal Header */}
            <div className="flex justify-between items-center p-4 border-b border-gray-200">
              <h2 className="text-lg font-semibold text-gray-900">
                Obesity Survey - {consultationInfo.patient?.user?.firstName} {consultationInfo.patient?.user?.lastName}
              </h2>
              <button
                onClick={() => setShowObesitySurvey(false)}
                className="text-gray-500 hover:text-gray-700 focus:outline-none"
              >
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            ...
            <div className="p-4 overflow-y-auto max-h-[70vh]">
              <div className="space-y-3">
                {consultationInfo.obesitySurvey.survey && consultationInfo.obesitySurvey.survey.length > 0 ? (
                  consultationInfo.obesitySurvey.survey.map((item: any, index: number) => (
